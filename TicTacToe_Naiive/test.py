...
def main():
	...
	for i in range(1, num_episodes+1):
		...
		while not done:
			...
			if training_switch:
				bot_train = not player_training
			else:
				bot_train = True
			#Step
			dummy, reward, done = env.step(action + 1, default_strat=default_strat, bot_train=bot_train)
		...
		if training_switch:
			if i % training_switch == 0:
				player_training = not player_training
	...
	plt.legend(loc='best')
	plt.savefig('Naiive_QLearning_Tic_Tac_Toe_Self_Play_X_Wins.png')
	plt.show()
